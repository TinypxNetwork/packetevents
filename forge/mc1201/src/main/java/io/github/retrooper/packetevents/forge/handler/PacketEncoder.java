...
package io.github.retrooper.packetevents.forge.handler;
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        ...
        if (didWeCauseThis && (user == null || user.getEncoderState() != ConnectionState.HANDSHAKING)) {
            if (PacketEvents.getAPI().getSettings().isKickOnPacketExceptionEnabled()) {
                ...
                ctx.channel().close();
                if (player != null) {
                    ((Forge1201PlayerManager) PacketEvents.getAPI().getPlayerManager()).kickOnException(player, "Invalid packet");
                }
            }
        }
